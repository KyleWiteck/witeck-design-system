...
| `color.magnitPrimary` | `string` | '#293F96' | - |
...
• **config**: [`addUniqueOptionBasedOnKeyConfig`](index.md#adduniqueoptionbasedonkeyconfigopt)\<`T`\>
...
function composeEventHandlers<E>(
   ...
   ourEventHandler?, 
   checkForDefaultPrevented?): (event) => void
```
...
Whether to check if `event.defaultPrevented` is set before calling `ourEventHandler`.

#### Returns

`Function`

A function that calls both handlers in order, respecting `defaultPrevented` if specified.
...
• **C** *extends* [`ErrorCatcherComponent`](index.md#errorcatchercomponente)\<`ErrorType`\>
